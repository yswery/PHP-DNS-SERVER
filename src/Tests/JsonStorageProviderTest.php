...
class JsonStorageProviderTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->storage = new JsonStorageProvider(__DIR__.'/data/dns.records.json');
    }
    ...
    public function testConstructorThrowsExceptions()
    {
        ...
        $jsonAdapter = new JsonStorageProvider(__DIR__.'/data/dns.invalid.json');

        $this->setExpectedException('\InvalidArgumentException', 'Default TTL must be an integer.');
        $jsonAdapter = new JsonStorageProvider(__DIR__.'/data/dns.records.json', '300');
    }
